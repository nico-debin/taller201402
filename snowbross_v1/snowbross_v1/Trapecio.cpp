...
Trapecio::Trapecio(b2World* mundo, float x,float y, float baseMayor,float baseMenor, float altura, string color,float rotacion, float masa, bool estatico):Figura(mundo,x,y,color,rotacion,estatico){
	
	this->datos.baseMayor = baseMayor;
	this->datos.baseMenor = baseMenor;
	this->datos.alto = altura;

	b2Vec2 vertices[4];
	vertices[0].Set(baseMenor/2,altura/2);
	vertices[1].Set(-baseMenor/2,altura/2);
	vertices[2].Set(-baseMayor/2,-altura/2);
	vertices[3].Set(baseMayor/2,-altura/2);

	b2PolygonShape trapecioShape;
	trapecioShape.Set(vertices,4);
