...
void Camara::actualizarCamaraXY(sEvento &estadoSnowBoy){
	...
	int diferenciaYSnowboyConCentroPantalla = ySnowBoyZoomeada - (this->y +Utilities::ALTO_PX/2);
...
SDL_Rect* Camara::getRectangulo(int anchoImagen, int altoImagen){
	...
	clip->y = (this->y-this->yMin)*relacionCamaraImagenY;
...
void Camara::actualizarAnchoAltoEscenarioPX(){
	...
	this->yMin = Utilities::ALTO_PX - this->altoEscenarioPX;
...
void Camara::aplicarZoomScrollYDibujar(Vista* vista, SDL_Renderer* renderer){
	...
	//ESTO ES UNA SOLUCION PARCIAL PARA EL PROBLEMA QUE TRAE CREAR UN FIGURA EN UN ZOOM QUE YA SUCEDIÓ, ANDABA BIEN ANTES CUANDO TODO SE CREABA AL MISMO TIEMPO PERO AHORA NO FUNCIONA PARA LAS NUEVAS COSAS QUE SE CREAN
	//PASA LO MISMO CUANDO EL ENEMIGO SE TRANSFORMA EN BOLA DE NIEVE
	if ( (vista->getTipo() != PROYECTIL) && (vista->getTipo() != ENEMIGO)&& (vista->getTipo() != VELOCIDAD)&& (vista->getTipo() != VIDA)){	
