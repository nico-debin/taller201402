...
vector<sEvento*>* Escenario::getEventosEscenario(){
...
sEvento* Escenario::crearEventoFigura(Figura* figura){
    ...
    if( figura->getTipo() == TIPO_SNOWBOY ){
		...
		evento->evento = ((SnowBoy*)figura)->getEstado();

    }else{
		   //evento->evento = EVENTO_ACTUALIZAR_FIGURA;
	...
	if( figura->getTipo() == TIPO_RECTANGULO ){
		((Rectangulo*)figura)->getVertices(evento->verticesX, evento->verticesY);
	}

	if( figura->getTipo() == TIPO_PARALELOGRAMO ){
		((Paralelogramo*)figura)->getVertices(evento->verticesX, evento->verticesY);
	}

	if( figura->getTipo() == TIPO_TRAPECIO ){
		((Trapecio*)figura)->getVertices(evento->verticesX, evento->verticesY);
	}

	for( short i = 0; i < MAX_LADOS ; i++ ){
		evento->verticesX[i] = Utilities::unidadLogica2Pixel( evento->verticesX[i], Utilities::ANCHO_PX, Utilities::ANCHO_UL);
		evento->verticesY[i] = Utilities::euclid2sdl_ejeY( Utilities::unidadLogica2Pixel( evento->verticesY[i], Utilities::ALTO_PX, Utilities::ALTO_UL) );
	}
