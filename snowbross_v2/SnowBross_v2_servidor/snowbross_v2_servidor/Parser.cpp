...
sEscenario Parser::parsearEscenario(Json::Value jsonEscenario){
	...
	//	parsearPersonajes(jsonEscenario.get("personajes",NULL), &escenario.objetos);
		parsearPersonajes(&escenario.objetos);
...
//void Parser::parsearPersonajes(Json::Value jsonPersonajes, vector<sObjeto>* objetos){
	...
	structPersonaje.x = 50;// SNOWBOY_ANCHO_UL + rand() % divisor;
	structPersonaje.y = 50;// SNOWBOY_ALTO_UL/2;
	...
	for (int i = 0; i < Utilities::MAX_JUGADORES; i++) {
		...
		structPersonaje.x = SNOWBOY_ANCHO_UL + (i*SNOWBOY_ANCHO_UL) + 2;// SNOWBOY_ANCHO_UL + rand() % divisor;
